...
export default async function RepaymentPage({ 
  ...
  const obligations = await getActiveDebtsGrouped(debtType)
  ...
  return (
    <main className="flex flex-col min-h-screen bg-zinc-50 relative pb-24">
      {/* Header */}
      <header className={`${isLent ? 'bg-orange-500' : 'bg-red-500'} text-white px-5 pt-8 pb-14 shadow-sm rounded-b-[2.5rem] relative z-20`}>
        <div className="flex items-center mb-6">
          <Link href={`/obligations${isLent ? '?tab=lent' : ''}`} className="p-2 -ml-2 rounded-full hover:bg-white/10 transition-colors">
            <ChevronLeft size={32} />
          </Link>
          <h1 className="text-xl font-bold ml-2 tracking-wide uppercase truncate">
            {isLent ? 'Collect Payment' : 'Repay'}
          </h1>
        </div>

        <div className="px-2">
          <h2 className="text-white/80 text-sm font-bold uppercase tracking-widest mb-1 truncate">{target.name}</h2>
